...
import rain.core.session.DefaultWriteRequestQueue;
...
/**
 * A default implementation of {@link IoFilterChain} that provides all
 * operations for developers who want to implement their own transport layer
 * once used with {@link AbstractIoSession}.
 *
 * @author <a href="http://mina.apache.org">Apache MINA Project</a>
 */
public class DefaultIoFilterChain implements IoFilterChain {
	/**
	 * A session attribute that stores an {@link IoFuture} related with the
	 * {@link IoSession}. {@link DefaultIoFilterChain} clears this attribute and
	 * notifies the future when {@link #fireSessionCreated()} or
	 * {@link #fireExceptionCaught(Throwable)} is invoked.
	 */
			...
			if (entry.getFilter() == oldFilter) {
				...
				try {
					...
					throw new IoFilterLifeCycleException(
							"onPreAdd(): " + oldFilterName + ':' + newFilter + " in " + getSession(), e);
				}
				...
				try {
					...
					entry.setFilter(oldFilter);
					throw new IoFilterLifeCycleException(
							"onPostAdd(): " + oldFilterName + ':' + newFilter + " in " + getSession(), e);
				}
	...
	/**
	 * Register the newly added filter, inserting it between the previous and the
	 * next filter in the filter's chain. We also call the preAdd and postAdd
	 * methods.
	 */
	...
	/**
	 * Throws an exception when the specified filter name is not registered in this
	 * chain.
	 *
	...
	/**
	 * Checks the specified filter name is already taken and throws an exception if
	 * already taken.
	 */
	...
	private class HeadFilter extends IoFilterAdapter {
		...
		public void filterWrite(NextFilter nextFilter, IoSession session, WriteRequest writeRequest) throws Exception {
			...
			DefaultWriteRequestQueue writeRequestQueue = s.getWriteRequestQueue();
