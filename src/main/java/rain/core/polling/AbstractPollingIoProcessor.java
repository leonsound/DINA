...
import rain.core.session.SessionState;
import rain.core.write.WriteRequest;
import rain.core.write.WriteToClosedSessionException;
import rain.transport.socket.AbstractDatagramSessionConfig;
...
public abstract class AbstractPollingIoProcessor<S extends AbstractIoSession> implements IoProcessor<S> {
    ...
    public void write(S session, WriteRequest writeRequest) {
    	DefaultWriteRequestQueue writeRequestQueue = session.getWriteRequestQueue();
    ...
    private class Processor implements Runnable {
        ...
        public void run() {
            ...
            for (;;) {
                try {
                    ...
                    if (!wakeupCalled.getAndSet(false) && (selected == 0) && (delta < 100)) {
                        ...
                        if (isBrokenConnection()) {
                            ...
                            if (nbTries == 0) {
                                ...
                                System.out.println("Create a new selector. Selected is 0, delta = " + delta);
                                registerNewSelector();
                                nbTries = 10;
                            } else {
                                nbTries--;
                    ...
                    System.out.println("ClosedSelectorException break ***");
                    break;
                } catch (Exception e) {
        ...
        private void updateTrafficMask() {
            ...
            while (queueSize > 0) {
                ...
                case OPENING:
                	...
                	System.out.println("trafficControllingSessions.add(session);");
                    trafficControllingSessions.add(session);
                    break;
        ...
        private boolean flushNow(S session, long currentTime) {
            ...
            final DefaultWriteRequestQueue writeRequestQueue = session.getWriteRequestQueue();
        ...
        private void process(S session) {
        	System.out.println("Deal with session ready for the read or write operations, or both. session:"+session);
            // Process Reads
